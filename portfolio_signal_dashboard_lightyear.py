...
def compute_leadlag_signal(symbol: str, relationships: pd.DataFrame, returns_panel: pd.DataFrame, cfg: StrategyConfig) -> Tuple[float, str]:
    ...
    for _, rel in rows.iterrows():
        ...
        if abs(bonus) > abs(best_bonus):
            ...
            best_reason = f"strict lead-lag: {leader} {arrow} {lag}d ago on an outsized move (z≈{z:.2f}); follower tends to move {direction}"
...
def run_streamlit_app() -> None:
    ...
    st.caption("Igapäevane ostu-, hoidmis- ja müügiettepanekute töölaud koos Lightyear-filtri, automaatkäivituse ja lead-lag lisasignaaliga.")
    ...
    with st.sidebar:
        ...
        max_new_positions_per_day = st.number_input("Maksimaalne uute positsioonide arv päevas", min_value=1, max_value=20, value=int(settings.get("max_new_positions_per_day", DEFAULT_MAX_NEW_POSITIONS_PER_DAY)), step=1)
        ...
        if st.button("Nulli portfell", use_container_width=True):
            ...
            st.warning("Portfell nulliti. Lae leht uuesti või vajuta Run daily scan.")
    ...
    i2.info(f"Skännitav universum: {len(filtered_universe)}")
    i3.info(f"Viimane scan: {settings.get('last_scan_date') or 'puudub'}")

    st.caption("Kui soovid ranget Lightyear filtrit, pane täpne tickerite nimekiri faili portfolio_state/lightyear_universe.csv.")
    ...
    if proposals.empty:
        st.write("Täna ettepanekuid veel ei ole. Vajuta **Run daily scan**.")
    else:
        ...
        with buy_tab:
            ...
            if df.empty:
                st.write("Täna uusi ostukandidaate ei ole.")
        ...
        with sell_tab:
            ...
            if df.empty:
                st.write("Täna müügiettepanekuid ei ole.")
            for _, row in df.iterrows():
                sym = str(row["symbol"])
                with st.container(border=True):
                    a, b, c = st.columns([2, 2, 1])
        ...
        with hold_tab:
            ...
            if df.empty:
                st.write("Avatud positsioonidele HOLD märget täna ei ole.")
            for _, row in df.iterrows():
                sym = str(row["symbol"])
                with st.container(border=True):
                    a, b, c = st.columns([2, 2, 1])
    ...
    if relationships.empty:
        st.write("Lead-lag seoseid veel ei ole või moodul on välja lülitatud.")
    else:
